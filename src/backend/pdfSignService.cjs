...
class HardwareSigner extends Signer {
    constructor(slotId, keyId, certDerBase64, chainB64Array = []) {
        super();
        this.slotId = slotId;
        this.keyId = keyId;
    ...
    async sign(pdfBuffer) {
        ...
        const customKey = {
            sign: function (md) {
                ...
                const signResult = pkcs11Service.signData(self.slotId, digestInfo, self.keyId);
...
async function signBuffer(pdfBuffer, slotId, keyId, certDerBase64, chainB64Array = [], signatureBox = null, iconName = 'ipe.png') {
    try {
        ...
        let icpImg = null;
        try {
            const safeIconName = iconName || 'ipe.png';
            const imgBuf = fs.readFileSync(__dirname + '/' + safeIconName);
            icpImg = await pdfDoc.embedPng(imgBuf);
        } catch (e) {
            console.log(`Logomarca ${iconName} não disponível, desenhando sem a logo visual.`);
        }
        ...
        const signer = new HardwareSigner(slotId, keyId, certDerBase64, chainB64Array);
