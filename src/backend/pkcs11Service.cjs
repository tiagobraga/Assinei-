...
class Pkcs11Service {
    constructor() {
        ...
        // Dinamicamente resolve o driver do SERPRO-ID com base no sistema operacional
        if (process.platform === 'darwin') {
            this.libPath = '/Applications/SerproID Desktop.app/Contents/Java/tools/macos/libneoidp11.dylib';
        } else if (process.platform === 'win32') {
            // Tenta os caminhos comuns no Windows
            this.libPath = fs.existsSync('C:\\Windows\\System32\\libneoidp11.dll') 
                ? 'C:\\Windows\\System32\\libneoidp11.dll' 
                : 'C:\\Program Files\\SERPRO\\SERPRO-ID\\libneoidp11.dll'; // Caminho alternativo genérico
        } else {
            // Linux padrão
            this.libPath = '/usr/lib/libneoidp11.so';
        }
                ...
                try {
                    ...
                    while (obj) {
                        try {
                            ...
                            const certIdVal = val[2].value; // CKA_ID
                            let certIdHex = '';
                            if (certIdVal) {
                                certIdHex = certIdVal.toString('hex');
                            }

                            if (certDer) {
                                allDerCerts.push({
                                    b64: certDer.toString('base64'),
                                    id: certIdHex
                                });
                            }
                        } catch (err) {
                    ...
                    for (const certObj of allDerCerts) {
                        const certB64 = certObj.b64;
                        const certIdHex = certObj.id;
    ...
    signData(slotIdStr, dataBuffer, keyIdHex, pin = '') {
        try {
            this.loadLibrary();
            ...
            // Achar chave privada correspondente ao CKA_ID (se disponibilizado pelo frontend)
            const searchTemplate = [
                { type: pkcs11js.CKA_CLASS, value: pkcs11js.CKO_PRIVATE_KEY }
            ];

            if (keyIdHex) {
                // Filtramos especificamente na chave que match o CKA_ID do Certificado
                searchTemplate.push({ type: pkcs11js.CKA_ID, value: Buffer.from(keyIdHex, 'hex') });
            }

            this.pkcs11.C_FindObjectsInit(session, searchTemplate);
