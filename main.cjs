...
ipcMain.handle('sign-pdf', async (event, args) => {
  try {
    ...
    // Chamada atualizada com o Array chainB64 e a BoundingBox Visual desenhada no React
    const signResult = await pdfSignService.signBuffer(pdfBuffer, args.slotId, args.certDerBase64, args.chainB64, args.signatureBox);
